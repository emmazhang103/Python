...
y_index=[i for i in range(len(sheet1.index))]#获取纵轴有多少行，作为y轴辅助绘图数据
y = [[i for y_data in range(len(x))] for i in range(1,len(y_index)+1)]#生成矩阵数据
...
for index in range(len(y_index)):#对转置后的每一列进行循环
    for i,j,r in zip(x,y[index],sheet1.T[sheet1.index[index]]):#循环执行，每次从x,y[index]中读取一个数，从转置后的excel表格中读取一列中的一个数据
        ...
        if np.sqrt(r/np.pi)/30 < 0.4:
            ax.pie(sheet['数量'], explode=None, labels=None, colors='w',
      autopct='%d', shadow=False, startangle=90,radius=1/2,center=(i, j),frame=True,textprops={'fontsize':8})#绘制散饼图,d%整数，f%浮点
